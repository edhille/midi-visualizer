...
function processWash(state, animEvent) {
   ...
   return [
      new D3AnimEvent({
         ...
         track: animEvent.track,
         color: setColor(state, animEvent),
         rotation: scales[rScale](r(animEvent)),
         x: state.width / 2,
         y: 0
      })
   ];
...
function processHorns(state, animEvent) {
   var rScale = state.height < state.width ? 'x' : 'y',
       scales = state.scales[animEvent.track],
       color = setColor(state, animEvent),
       rad = scales[rScale](r(animEvent)) / 2,
       y = state.height / 2;
...
function processKeys(state, animEvent) {
   var rScale = state.height < state.width ? 'x' : 'y',
       scales = state.scales[animEvent.track],
       color = setColor(state, animEvent),
       rad = scales[rScale](r(animEvent)) / 4,
       lx = state.width / 4,
       rx = state.width - lx,
       ty = state.height / 4,
       by = ty;
...
function processLead(state, animEvent) {
   ...
   return [
      new D3AnimEvent({
         ...
         track: animEvent.track,
         color: setColor(state, animEvent),
         radius: scales[rScale](r(animEvent)) / 2,
         x: state.width / 2,
         y: state.height / 2
      })
   ];
  ...
  return Object.keys(state.animEvents).reduce(function _transform(acc, time) {
     var events = state.animEvents[time],
         processedEvents = events.reduce(function _process(acc, event) {
            return acc.concat(processEvent(state, event));
         }, []);
