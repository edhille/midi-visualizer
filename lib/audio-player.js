...
AudioPlayer.prototype.loadData = function loadData(audioData, callback) {
   ...
   return new Promise(function _loadData(resolve, reject) {

      if (!existy(audioData)) {
         reject(new Error('must provide an AudioData source'));
      }

      if (self.isLoading) {
         resolve('Already loading audio data');
      }
      ...
      try {
         ...
         reject(new Error('error decoding audio: ' + e.message));
      }
      ...
      function setupAudioSource(buffer) {
         ...
         // TODO: if this was truly functional, this would be a new instance
         resolve(self);
      }
   });
};
