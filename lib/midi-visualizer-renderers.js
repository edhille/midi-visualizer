...
function getY(d, state) {
   return state.scales[d.track].y(d.y);
}

function getX(d, state) {
   return state.scales[d.track].x(d.x);
}

function getR(d, state) {
   return state.scales[d.track].note(d.radius);
}
...
function transform(datum, state) {
   var x = getX(datum, state),
       y = getY(datum, state),
       box, halfWidth, halfHeight, targetWidth, scale;

   console.log('d.x', datum.x, 'd.y', datum.y, 'x', x, 'y', y);
   ...
   switch (this.tagName) {
      ...
      case 'path':
         ...
         halfHeight = state.height/ 2;
         targetWidth = (halfWidth / 2) + box.width;

         scale = targetWidth/box.width;

         console.log(state.width + ',' + state.height, 'halfWidth', halfWidth, 'halfHeight', halfHeight, 'scale', scale);
         // this.setAttribute('transform', 'matrix(' + scale + ',0,0,' + scale + ',' + (x-halfWidth) + ',' + (y-halfWidth) + ')');
         this.setAttribute('transform', 'matrix(' + scale + ',0,0,' + scale + ',' + (x-targetWidth) + ',' + (y-halfHeight) + ')');
         break;
      default:
...
function sizeElem(datum, state) {
   ...
   switch (this.tagName) {
      case 'circle':
         this.setAttribute('r', getR(datum, state));
         break;
      case 'path':
         break;
      default:
...
function renderD3(state, renderEvents) {
   ...
   enter.each(function (datum) { return transform.call(this, datum, state); });
   enter.each(function (datum) { return sizeElem.call(this, datum, state); });
