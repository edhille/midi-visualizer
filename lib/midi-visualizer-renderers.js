...
function trackEventFilter(event) {
    return true; // event instanceof MidiNoteEvent || event instanceof MidiMetaTempoEvent;
}
    ...
    var tmp = midi.tracks.reduce(function _reduceTrack(eventsByTime, track, trackIndex) {
        ...
        return track.events.reduce(function _reduceEvent(eventsByTime2, event) {
            ...
            if (event instanceof MidiMetaTempoEvent) {
                ...
                if (event instanceof MidiNoteOnEvent) {
                   ...
                   activeNotes[event.note].push({ event: event, startTime: elapsedTimeInMicroSec, index: 0 });
                    ...
                    if (startNote) {
                       ...
                       // NOTE: this is happening on note 57
                       //       track: 6 
                       //           on  notes: 98 
                       //           off notes: 98 
                       //       99991200 microsec is the prev "on" note
                       //       it is the exact same note twice...
                       //       there is one inserted w/ index 3
                       //       there is one inserted w/ index 4
                       //       so questions are.....
                       //           can you have two on events before an off one? YES
                       //           is there really two on notes in the original midi file? MOST LIKELY
                       //                 these could be overlapping, duplicate notes (which you can create in beatmaker...)
                ...
                if (event instanceof MidiNoteEvent && (!activeNotes[event.note] || activeNotes[event.note].length <= 1)) {
