...
var d3 = require('d3'),
    ...
    ThreeJsRenderState = eventTypes.ThreeJsRenderState;
...
function scheduleD3AnimEvents(renderFn, state, startOffset) {
    ...
    return new D3RenderState({
       ...
       renderEvents: setTimers(renderFn, state, startOffset),
...
function minNote(currMinNote, event) {
   return currMinNote < event.event.note ? currMinNote : event.event.note;
}
...
/**
 * D3 Renderer base utilities (TODO: move to own file)
 */
...
function accumulateScales(width, height, acc, events, index) {
   var scale = {
      ...
      hue: d3.scale.linear().range([0,360]).domain([0,8]),
      velocity: d3.scale.linear().range([30,60]).domain([0, 256])
   };

   var maximumNoteValue = events.reduce(maxNote, 0);
   var minimumNoteValue = events.reduce(minNote, maximumNoteValue);

   scale.y.range([25, height]);
   scale.y.domain([minimumNoteValue, maximumNoteValue]);

   scale.x.range([25, width]);
   scale.x.domain([minimumNoteValue, maximumNoteValue]);

   scale.note.range([0, width < height ? height : width]);
   scale.note.domain([minimumNoteValue, maximumNoteValue]);
...
function scaleD3(state, width, height) {
   ...
   scales.unshift({}); // HACK: to get indices to line up between scales and tracks
...
d3Renderer.lift('scheduleAnimation', partial(scheduleD3AnimEvents, renderD3));

/**
 * ThreeJS Renderer base utilities (TODO: move to own file)
 */

function prepThreeJsDOM(state, midi) {
   var scene = new THREE.Scene();
   var camera = new THREE.PerspectiveCamera( 75, state.width / state.height, 0.1, 1000 );
   var renderer = new THREE.WebGLRenderer();
   var animEvents = transformMidi(midi);

   renderer.setSize(state.width, state.height);

   state.root.appendChild(renderer.domElement);

   state = new ThreeJsRenderState({
       root: state.root,
       width: state.width,
       height: state.height,
       animEvents: animEvents,
   ...
   return scaleThreeJs(state, state.width, state.height);
}

function resumeThreeJs(state) {
...
function scaleThreeJs(state, width, height) {
   ...
   //       hopefully this is not triggered much (should only happen on a
   //       resizing of the root), so it's okay...
   // TODO: abstract this for reuse between implementations
   var animEvents = [].concat.apply([], Object.keys(state.animEvents).map(function (time) { return state.animEvents[time]; }));
...
function scheduleThreeJsAnimEvents(renderFn, state, startOffset) {
...
function renderThreeJs(state, renderEvents) {
   // TODO: a lot of this is common with renderD3...
   var currentRunningEvents = state.currentRunningEvents;
   var l = renderEvents.length;
   var i = 0;
   var index = -1;
...
threeJsRenderer.lift('render', renderThreeJs);
threeJsRenderer.lift('scheduleAnimation', partial(scheduleThreeJsAnimEvents, renderThreeJs));
...
module.exports = {
   ...
   threejs: {
      ...
      utils: {
         render: renderThreeJs,
         prep: prepThreeJsDOM,
         scale: scaleThreeJs,
         resume: resumeThreeJs
      }
   }
};
