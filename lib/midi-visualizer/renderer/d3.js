...
function transform(datum) {
   var x = getX(datum),
       y = getY(datum),
       box, midpoint, targetWidth, scale;
   ...
   switch (this.tagName) {
      ...
      case 'path':
         box = this.getBBox();
         midpoint = getR(datum);
         targetWidth = midpoint * 2;

         // TODO: this scale is not actually right (though we are centering correctly when using it)
         scale = targetWidth/box.width;

         this.setAttribute('transform', 'matrix(' + scale + ',0,0,' + scale + ',' + (x-midpoint) + ',' + (y-midpoint) + ')');
         break;
      default:
