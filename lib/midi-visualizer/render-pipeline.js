 ...
 *  
 *  prepResume() - do any manipulation necessary to prepare for animation resuming
 *
...
function midiRenderPipeline(config) {
   ...
   var pipeline = {
      ...
      prepResume: function prepResume() {
         return renderer.prepResume.apply(renderer, arguments);
      },
      transformMidiData: function transformMidiData(time, midiData) {
         var animData = [],
            stackIndex = 0,
         ...
         function next() {
            ...
            // HACK: I don't like this...need to figure out a better way
            //       to ensure that animation data is chained through
            //       all the filters...
            animData = args[2];

            if (stackIndex < filtersCount) {
               return filters[stackIndex++].apply(renderer, args);
            } else {
               return animData;
            }
         }
         ...
         if (stackIndex < filtersCount) {
            return next(null, time, midiData, animData);
         }
      },
