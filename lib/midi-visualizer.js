...
module.exports = function run(config) {
   loadData(config).then(function (data) {
      ...
      audioPlayer.loadData(audioData).then(function (audioPlayer) {
         ...
         // TODO: this renderer needs to be a song-specific renderer which can position/size/color animEvents...
         var renderer = renderers.d3(new RenderState(root, width, height)).transformMidi(midi).prep(midi);
