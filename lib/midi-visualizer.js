...
var utils = require('funtils'),
    ...
    // TODO: this should be configurable based upon the renderer
    RenderState = types.ThreeJsRenderState,
...
function loadHandler(configRenderer, data) {
   ...
   audioPlayer.loadData(audioData).then(function (audioPlayer) {
      // TODO: get window from input (pass down from config?)
      var w = window;
      var d = w.document;
      ...
      try {
         ...
         var state = new VisualizerState({
            ...
            renderer: configRenderer(renderState).prep(midi)
         });
         ...
         w.addEventListener('resize', debounce(function (e) {
            console.log('resize...', e);
            visualizer = visualizer.resize(w.innerWidth, w.innerHeight);
         }, 1000));
...
module.exports = function run(config) {
   // TODO: fix so it's not config.config
