...
function runVisualization(midiVisualizer, startOffset) {
   ...
   console.log('playhead', playheadTimeInMs);
...
function MidiVisualizer(params) {
   ...
   this.timeAtPauseInSec = 0; // track the pause point

   // TODO: maybe the audioplayer should manage this (since it's specific to the WebAudio API's AudioContext.currentTime being a total lifetime timer rather than a play-time timer...)
   ...
   // TODO: these should more likely be managed by the audioplayer since we're syncing with that
   hideProperty(this, 'timeAtPauseInSec');
   hideProperty(this, 'totalPauseInSec');
}
